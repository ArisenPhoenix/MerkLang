...
#include "core/Environments/Environment.hpp"
...
class Scope : public Environment, public ScopeMeta, public std::enable_shared_from_this<Scope> {
private:
...
public:
    ...
    explicit Scope(SharedPtr<Scope> parentScope, SharedPtr<FunctionRegistry> globalF, SharedPtr<ClassRegistry> globalC, SharedPtr<TypeRegistry> globalT, bool interpretMode);
    // Root scope with pre-existing registries (for clone); does not create new empty registries
    explicit Scope(SharedPtr<FunctionRegistry> globalF, SharedPtr<ClassRegistry> globalC, SharedPtr<TypeRegistry> globalT, SharedPtr<TypeSignatureRegistryManager> globalTypeSigs, bool interpretMode);

    // // DESTRUCTOR
    ~Scope();
    virtual void clear(bool internalCall = true);
    ...
    virtual SharedPtr<Scope> clone(bool strict = false) const;
