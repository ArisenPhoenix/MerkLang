...
Scope::Scope(WeakPtr<Scope> parentScope, bool interpretMode)
  : parentScope(std::move(parentScope)),
    interpretMode(interpretMode)
{
    if (auto parent = this->parentScope.lock()) {
        ...
        if (!globalTypes)    throw MerkError("Scope weak-parent ctor: globalTypes is null");
        if (!globalTypeSigs) throw MerkError("Scope weak-parent ctor: globalTypeSigs is null");
...
// Root scope with pre-existing registries (for clone)
Scope::Scope(SharedPtr<FunctionRegistry> globalF, SharedPtr<ClassRegistry> globalC, SharedPtr<TypeRegistry> globalT, SharedPtr<TypeSignatureRegistryManager> globalTypeSigs, bool interpretMode)
  : interpretMode(interpretMode),
    globalFunctions(std::move(globalF)),
    globalClasses(std::move(globalC)),
    globalTypes(std::move(globalT)),
    globalTypeSigs(globalTypeSigs ? std::move(globalTypeSigs) : (globalT ? makeShared<TypeSignatureRegistryManager>(*globalT) : nullptr))
{
    isRoot = true;
    scopeLevel = 0;
    if (!globalTypes) throw MerkError("Scope root-with-registries ctor: globalTypes is null");
    if (!this->globalTypeSigs) throw MerkError("Scope root-with-registries ctor: globalTypeSigs is null");
    initRootTypes();
    ++liveScopeCount;
    ++totalScopeCreated;
}
...
void Scope::clear(bool internalCall) {
    ...
    // Only for Root Scope - do not clear shared registries when this is a cloned scope
    // (eval-only mode reuses the same globalFunctions/globalClasses across many evals)
    if (isRoot && !isClonedScope) {
