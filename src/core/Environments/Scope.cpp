...
SharedPtr<Scope> Scope::getParent() const {
    ...
    DEBUG_FLOW_EXIT();
    if (kind != ScopeKind::Root && !isRoot) {
        throw ParentScopeNotFoundError("getParent called on non-root scope: " + metaString());
    }
...
bool Scope::tryReadInt(const String& name, int& out) const {
    for (const Scope* s = this; s != nullptr; ) {
        if (const VarNode* variable = s->context.findVariable(name)) {
            const Node& n = variable->getValueNode();
...
bool Scope::tryWriteInt(const String& name, int value) {
    for (Scope* s = this; s != nullptr; ) {
        if (VarNode* var = s->context.findVariable(name)) {
            DataTypeFlags& vf = var->getVarFlags();
            if (vf.isConst) {
                throw MerkError("Cannot assign to const variable '" + name + "'");
            }

            auto& tsr = s->localTypes;
            ...
            Node n(value);
            if (n.getFlags().isMutable != vf.isMutable) {
                n.getFlags().isMutable = vf.isMutable;
            }
            s->context.updateVariable(name, n);
            return true;
        }
        auto parent = s->getParent();
        s = parent.get();
