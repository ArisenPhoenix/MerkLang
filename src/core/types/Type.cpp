...
TypeMatchResult TypeMatchResult::No() {
    return {};
}

TypeMatchResult TypeMatchResult::Yes(int score_, int cost_) {
    return {true, score_, cost_};
}
...
TypeSigKind PrimitiveType::kind() const { return TypeSigKind::Primitive; }
...
TypeMatchResult PrimitiveType::matchValueInternal(
    ...
    if (opt.allowNumericWidening &&
        TypeRegistry::isNumeric(vt) &&
        TypeRegistry::isNumeric(prim) &&
        TypeRegistry::numericRank(vt) <=
        TypeRegistry::numericRank(prim)) {
...
TypeSigKind ClassType::kind() const { return TypeSigKind::Class; }
...
TypeMatchResult ClassType::matchValueInternal(
    ...
    return TypeMatchResult::No(); // TODO: nominal instance check
}
...
TypeSigKind MethodType::kind() const { return TypeSigKind::Method; }

String MethodType::describe(const TypeRegistry& reg) const {
    String s;
...
TypeMatchResult MethodType::matchCallInternal(
